...
export default async function validateDesignElementHandler(
    ...
    const { controlId, designElementId, prompt, question, evidences } = req.body;


    if (
        typeof controlId !== 'string' ||
        typeof designElementId !== 'string' ||
    ...
    try {
        ...
        return res.status(200).json({ designElementId, answer });
    } catch (error) {
